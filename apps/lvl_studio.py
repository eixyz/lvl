...
# NOTE: project create/open/persist now lives in src.io.project_io (Project,
# create_project, open_project, ...). LvlStudioWindow drives it from the
# File menu - see _new_project_dialog / _open_project_dialog below.
...
class LvlStudioWindow(QtWidgets.QMainWindow):
    ...
    def _require_project(self) -> "pio.Project | None":
        ...
        Call this at the top of any action (opening SEG2 data, saving
        picks, running computation, ...) that needs somewhere to write
        project output.
        """
    ...
    def _new_project_dialog(self):
        ...
        parent_dir = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Folder to create the project in",
            str(core_paths.PROJECTS_ROOT),
        )
        if not parent_dir:
            return

        try:
            project = pio.create_project(name, parent_dir=Path(parent_dir))
            ...
            QtWidgets.QMessageBox.critical(self, "New Project", f"Could not create project:\n{exc}")
            return

        self._set_project(project)
        # Immediately offer to point the new project at its raw SEG2 data.
        self._edit_project_settings_dialog(prompt_raw_folder=True)

    def _open_project_dialog(self):
        start_dir = str(core_paths.PROJECTS_ROOT if core_paths.PROJECTS_ROOT.exists() else Path.cwd())
        folder = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Open Project (select its folder)", start_dir
        )
        if not folder:
            return
        try:
            project = pio.open_project(folder)
            ...
            QtWidgets.QMessageBox.critical(self, "Open Project", f"Could not open project:\n{exc}")
            return

        self._set_project(project)

    def _edit_project_settings_dialog(self, prompt_raw_folder: bool = False):
        project = self._require_project()
        if project is None:
            return

        if prompt_raw_folder or QtWidgets.QMessageBox.question(
            self, "Project Settings",
            "Set/change the raw SEG2 data folder for this project?",
        ) == QtWidgets.QMessageBox.StandardButton.Yes:
            start = str(project.raw_folder or Path.cwd())
            folder = QtWidgets.QFileDialog.getExistingDirectory(
        ...
        if QtWidgets.QMessageBox.question(
            self, "Project Settings",
            "Set/change the geometry/coordinate files folder for this project?",
        ) == QtWidgets.QMessageBox.StandardButton.Yes:
            start = str(project.geometry_folder or Path.cwd())
            folder = QtWidgets.QFileDialog.getExistingDirectory(
                self, "Geometry/coordinate files folder", start
            )
            if folder:
                project.geometry_folder = Path(folder).resolve()

        if QtWidgets.QMessageBox.question(
            self, "Project Settings",
            "Set/change the field-report metadata folder for this project?",
        ) == QtWidgets.QMessageBox.StandardButton.Yes:
            start = str(project.metadata_folder or Path.cwd())
            folder = QtWidgets.QFileDialog.getExistingDirectory(
                self, "Field-report metadata folder", start
            )
            if folder:
                project.metadata_folder = Path(folder).resolve()

        pio.save_project(project)
        self.statusBar().showMessage(f"Project '{project.name}' settings saved.")
    ...
    def _build_actions(self):
        ...
        act_new_project.triggered.connect(self._new_project_dialog)
    ...
    def _on_geometry_changed(self, text: str):
        ...
        if t == "custom":
            start_dir = str(GEOM_TEMPLATES_DIR if GEOM_TEMPLATES_DIR.exists() else Path.cwd())
    ...
    def open_folder(self):
        ...
        start_dir = str(raw_folder if raw_folder and raw_folder.exists() else Path.cwd())
    ...
    def _autoload_picks(self, profile_name: str) -> int:
        ...
        ap = core_paths.require_active_project()
        candidates = (
            ap.session_picks_json(profile_name),
            ap.picks_json(profile_name),
        )
    ...
    def _discover_report_paths(self) -> list[Path]:
        ...
        search_dirs = [d for d in (
            self.project.metadata_folder if self.project else None,
            self.project.geometry_folder if self.project else None,
        ) if d is not None]
